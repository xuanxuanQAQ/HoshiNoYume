...
def vits_tts(_text):
    ...
    with no_grad():
        ...
        audio = net_g_ms.infer(x_tst, x_tst_lengths, sid=sid, noise_scale=.667,
                               noise_scale_w=.8, length_scale=1.)[0][0, 0].data.cpu().float().numpy()
    ...
    if _text != "":
        ...
        thread_translate = threading.Thread(
            target=youdao_translate, args=(_text,))
        if IoT_enabled:
            global mqtt_control
            if mqtt_control:
...
def main():
    ...
    while True:
        ...
        user_words, weather_data = asyncio.run(
            sec_api_request(wav_base64))  # 第二次API请求，通过异步并行运行以降低时耗
        if user_words == "":
            wait_to_wake_up()
            continue
        ...
        if index == -1:
            ...
            if text.rfind('@light') != -1 and IoT_enabled:
                ...
                thread_mqtt_publish = threading.Thread(
                    target=mqtt_publish, args=(user_words,))
